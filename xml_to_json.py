...
import xmltodict
"""
    - This class takes the name of an XML file as an argument passed by the
    command line. Then it creates or opens a file with the same name, but with
    the extension.JSON where the results will be stored. It first converts the
    xml file to a dictionary and converts the dictionary in to a JSON.
    - Sources:
